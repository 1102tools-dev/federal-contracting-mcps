# Acquisition.gov remote pilot

Full 5-tool deployment of the existing Python server. Source stdio behavior remains unchanged. No public deployment or OpenAI approval is implied by this package.
...
The production configuration binds `acquisition-gov.1102tools.com`. It does not replace the main Pages website. Validate the deployed endpoint using `review/live_check.py` and a supported MCP client before submitting it for review.

From the repository root:

```sh
uv run --project servers/acquisition-gov-mcp pytest servers/acquisition-gov-mcp/tests -q
uv run --project servers/acquisition-gov-mcp python deploy/acquisition-gov/review/live_check.py https://acquisition-gov.1102tools.com/mcp
```
...
Endpoint: https://acquisition-gov.1102tools.com/mcp

All 5 tools passed live source calls on September 7, 2026. 23 offline tests passed. See review/live-results.json and review/deployment.json. Icons include directory and composer light/dark variants. OpenAI recording and final attestations remain separate.

Build image from repository root with docker build --platform linux/amd64 -f deploy/acquisition-gov/Dockerfile. Wrangler points to the verified registry digest. Update the digest deliberately when rebuilding. The local deployment token cannot manage Containers; use the authenticated Cloudflare connector for registry credentials and application changes. Preserve containers metadata on Worker-only API uploads.
