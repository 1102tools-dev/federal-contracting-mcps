"""GSA CALC+-only, cross-process rolling budget and concurrent request pacing.
...
class CalcBudgetUnavailable(RuntimeError):
    """An expected pacing refusal with a safe, actionable retry message."""


class GsaCalcPacer(FederalApiPacer):
    MAX_REQUESTS = 500
    WINDOW_SECONDS = 3600.0
    MAX_IN_FLIGHT = 2

    def __init__(self, **kwargs):
        super().__init__(bucket="api.gsa.gov:calc", default_interval=0.6, **kwargs)
    ...
    def _budget_state(self, path):
        try:
            ...
            raise RuntimeError("GSA CALC+ pacing state is unreadable") from exc
        if not isinstance(state, dict):
            raise RuntimeError("GSA CALC+ pacing state is invalid")
        for key in ("last_completed", "last_started", "cooldown_until"):
            value = state.get(key, 0)
            if not isinstance(value, (int, float)) or not math.isfinite(value) or value < 0:
                raise RuntimeError("GSA CALC+ pacing timestamp is invalid")
        starts = state.get("starts", [])
        if not isinstance(starts, list) or any(
            not isinstance(v, (int, float)) or not math.isfinite(v) or v < 0
            for v in starts
        ):
            raise RuntimeError("GSA CALC+ pacing history is invalid")
        return state
    ...
    async def _reserve(self, interval):
        while True:
            async with self._state_lock() as path:
                ...
                delay = ready - now
                if len(starts) >= self.MAX_REQUESTS and delay > 0:
                    raise CalcBudgetUnavailable(
                        f"GSA CALC+ local safety budget of 500 upstream attempts per hour "
                        f"is exhausted. Retry after {math.ceil(delay)} seconds."
                    )
                if state.get("cooldown_until", 0) - now > 30:
                    raise CalcBudgetUnavailable(
                        f"GSA CALC+ provider cooldown is active. Retry after "
                        f"{math.ceil(state['cooldown_until'] - now)} seconds."
                    )
                if delay <= 0:
    ...
    async def cooldown_remaining(self):
        if self.configured_interval() == 0:
            return 0
        async with self._state_lock() as path:
            state = self._budget_state(path)
            return max(0, math.ceil(state.get("cooldown_until", 0) - self.clock()))
