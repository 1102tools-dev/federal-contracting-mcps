"""Federal Register-only, cross-process rolling budget and concurrent request pacing.
...
class FederalRegisterPacer(FederalApiPacer):
    MAX_REQUESTS = 500
    WINDOW_SECONDS = 300.0
    MAX_IN_FLIGHT = 2

    def __init__(self, **kwargs):
        super().__init__(bucket="www.federalregister.gov", default_interval=0.6, **kwargs)
    ...
    def _budget_state(self, path):
        try:
            ...
            raise RuntimeError("Federal Register pacing state is unreadable") from exc
        if not isinstance(state, dict):
            raise RuntimeError("Federal Register pacing state is invalid")
        for key in ("last_completed", "last_started", "cooldown_until"):
            value = state.get(key, 0)
            if not isinstance(value, (int, float)) or not math.isfinite(value) or value < 0:
                raise RuntimeError("Federal Register pacing timestamp is invalid")
        starts = state.get("starts", [])
        if not isinstance(starts, list) or any(
            not isinstance(v, (int, float)) or not math.isfinite(v) or v < 0
            for v in starts
        ):
            raise RuntimeError("Federal Register pacing history is invalid")
        return state
