"""USAspending-only, cross-process rolling budget and concurrent request pacing.

All clients sharing a pacing directory share the budget. State counts attempts,
not successful responses. File locks are held across I/O only for the four
...
class USASpendingPacer(FederalApiPacer):
    MAX_REQUESTS = 500
    WINDOW_SECONDS = 300.0
    MAX_IN_FLIGHT = 4

    def __init__(self, **kwargs):
        super().__init__(bucket="api.usaspending.gov", default_interval=0.6, **kwargs)
    ...
    def _budget_state(self, path):
        try:
            ...
            raise RuntimeError("USAspending pacing state is unreadable") from exc
        if not isinstance(state, dict):
            raise RuntimeError("USAspending pacing state is invalid")
        for key in ("last_completed", "last_started", "cooldown_until"):
            value = state.get(key, 0)
            if not isinstance(value, (int, float)) or not math.isfinite(value) or value < 0:
                raise RuntimeError("USAspending pacing timestamp is invalid")
        starts = state.get("starts", [])
        if not isinstance(starts, list) or any(
            not isinstance(v, (int, float)) or not math.isfinite(v) or v < 0
            for v in starts
        ):
            raise RuntimeError("USAspending pacing history is invalid")
        return state
