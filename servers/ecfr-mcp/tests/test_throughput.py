...
from ecfr_mcp._throughput import EcfrPacer
...
def pacer(tmp_path, **kw):
    return EcfrPacer(pacing_dir=tmp_path, environment={}, **kw)
...
async def test_positive_override_cannot_accelerate_default(tmp_path):
    clock=Clock()
    p=EcfrPacer(pacing_dir=tmp_path,environment={'FEDERAL_API_MIN_INTERVAL_SECONDS':'.1'},clock=clock,sleep=clock.sleep)
