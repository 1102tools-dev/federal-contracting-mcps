...
## API key (required for city lookups)

City lookups (`lookup_city_perdiem`, `estimate_travel_cost`,
`compare_locations`) call `api.gsa.gov`, which requires an api.data.gov key:
1,000 requests per hour, yours alone. Without `PERDIEM_API_KEY` they return
setup instructions instead of calling GSA. ZIP, state, and M&IE lookups for
bundled fiscal years need no key.
...
The server defaults `FEDERAL_API_MIN_INTERVAL_SECONDS` to `4` for keyed
requests. The explicit value above documents the intended policy
...
**Bundled lookups only** (no key: ZIP, state, and M&IE work; city lookups return setup instructions):
```json
{
  "mcpServers": {
    "gsa-perdiem": {
