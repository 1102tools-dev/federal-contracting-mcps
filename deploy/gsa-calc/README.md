...
The production configuration binds `gsa-calc.1102tools.com`. It does not replace the main Pages website. Validate the deployed endpoint using `review/live_check.py` and a supported MCP client before submitting it for review.

From the repository root:

```sh
uv run --project servers/gsa-calc-mcp pytest servers/gsa-calc-mcp/tests -q
uv run --project servers/gsa-calc-mcp python deploy/gsa-calc/review/live_check.py https://gsa-calc.1102tools.com/mcp
```
...
Endpoint: https://gsa-calc.1102tools.com/mcp

All 8 tools passed live source calls on September 7, 2026. 250 offline tests passed. See review/live-results.json and review/deployment.json. Icons include directory and composer light/dark variants. OpenAI recording and final attestations remain separate.

Build image from repository root with docker build --platform linux/amd64 -f deploy/gsa-calc/Dockerfile. Wrangler points to the verified registry digest. Update the digest deliberately when rebuilding. The local deployment token cannot manage Containers; use the authenticated Cloudflare connector for registry credentials and application changes. Preserve containers metadata on Worker-only API uploads.
