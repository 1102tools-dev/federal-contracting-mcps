...
def pacer(tmp_path, **kw):
    return USASpendingPacer(pacing_dir=tmp_path, environment={}, **kw)
...
async def test_cancellation_waiting_does_not_leak_slot(tmp_path):
    p=pacer(tmp_path)
    contexts=[p._concurrency_slot() for _ in range(4)]
...
async def test_positive_override_cannot_accelerate_default(tmp_path):
    clock=Clock()
    p=USASpendingPacer(pacing_dir=tmp_path,environment={'FEDERAL_API_MIN_INTERVAL_SECONDS':'.1'},clock=clock,sleep=clock.sleep)
...
async def test_cross_process_slots_and_crash_recovery(tmp_path):
    ...
    processes=[ctx.Process(target=child_slot,args=(str(tmp_path),q,release)) for _ in range(4)]
    try:
